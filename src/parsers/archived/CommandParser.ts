...
export class CommandParser {
    ...
    parseCommandFromTokens(tokens: string[], startLine?: number): CommandCall {
        ...
        while (i < tokens.length || pos < line.length || currentLineIndex < this.context.lines.length) {
            ...
            // Validate that we're actually at a '{' character before calling extractObjectLiteral
            if (pos < line.length && line[pos] === '{') {
                const startLineIndex = this.context.getCurrentLine();
                // Double-check that pos actually points to '{' (defensive programming)
                if (line[pos] !== '{') {
                    throw this.context.createError(`internal error: expected '{' at position ${pos}`, startLineIndex);
                }
            ...
            // But first check if this is array indexing (e.g., $var[1]) - if so, skip it
            // Array indexing comes after a variable, so check if the previous character is part of a variable
            if (pos < line.length && line[pos] === '[') {
                // First, check if the current token (or previous token) is a variable
                // Variables with array indexing are tokenized as a single VARIABLE token
                let isIndexing = false;
                
                // Check if we're currently inside or right after a variable token
                if (i > 0 && i <= tokens.length) {
                    // Check previous token or current token if we haven't advanced yet
                    const checkTokenIndex = i > 0 ? i - 1 : i;
                    if (checkTokenIndex < tokens.length) {
                        const token = tokens[checkTokenIndex];
                        // If the token starts with $ or contains [, it's likely a variable with indexing
                ...
                // Also check character-based: if previous character is part of a variable
                if (!isIndexing && pos > 0) {
                    const prevChar = line[pos - 1];
                    // Check if previous character is part of a variable (alphanumeric, _, or $)
                ...
                if (isIndexing) {
                    // This is array indexing, not an array literal - skip past the indexing brackets
                    // Find the matching closing bracket(s) - handle nested indexing like [1][2]
                    let bracketDepth = 1;
                    pos++; // Skip opening '['
                    while (pos < line.length && bracketDepth > 0) {
                        if (line[pos] === '[') bracketDepth++;
                        else if (line[pos] === ']') bracketDepth--;
                        pos++;
                    }
                    // Skip any tokens that were part of this indexing
                    while (i < tokens.length) {
                        const tokenStart = line.indexOf(tokens[i], Math.max(0, pos - 100));
                        if (tokenStart === -1 || tokenStart >= pos) {
                            break;
                    ...
                    // This is an array literal, not array indexing
                    const startLineIndex = this.context.getCurrentLine();
