    ...
    private async executeStatement(stmt: Statement, frameOverride?: Frame): Promise<void> {
        switch (stmt.type) {
            ...
            case 'comment':
                ...
                break;
            case 'chunk_marker':
                // Chunk markers are no-ops during execution
                break;
            case 'cell':
                // Cell blocks: execute body if cellType is "code"
                if (stmt.cellType === 'code' && stmt.body) {
                    await this.execute(stmt.body);
                }
                // Other cell types (prompt, schema, etc.) are no-ops
                break;
            case 'prompt_block':
                // Prompt blocks are no-ops during execution
                break;
        }
    }
